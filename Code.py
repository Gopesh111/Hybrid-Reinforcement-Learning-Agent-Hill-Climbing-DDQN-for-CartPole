...
27  
        ...
        print(f"HC Ep {ep+1} | Reward: {total_reward:.2f} | Accuracy: 
{accuracy*100:.2f}% | Best: {best_R:.2f}") 
...
accuracies.append(accuracy) 
agent.update_target() 
print(f"{desc} Ep {ep+1} | Reward: {total_reward:.2f} | Accuracy: 
{accuracy*100:.2f}% | Eps: {agent.epsilon:.3f}") 
if np.mean(scores[-100:]) >= 195: 
print(f"   
break 
{desc} solved in {ep+1} episodes!") 
...
ddqn_scores_combined, 
ddqn_accuracies_combined 
= 
train_ddqn(env, 
30 
...
Hill Climbing Only Accuracy     : {overall_hc_accuracy * 100:.2f}%") 
DDQN Only (no HC) Accuracy      : {overall_ddqn_accuracy * 100:.2f}%") 
DDQN + HC Prefill Accuracy      : {overall_combo_accuracy * 100:.2f}%") 
# Final animation 
watch_trained_agent(env, ddqn_agent_combo)

this is one more project, write this about it in my resume, ats friendly
